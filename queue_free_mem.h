...
public:
	...
	long int front_ex_p, back_ex_p;	// 16 bytes
	void **pMemList_Ex_Pointer=NULL;		// 8 bytes
	pthread_mutex_t lock_Ex_Pointer;	// 40 bytes

	long int front_Dir_Entry, back_Dir_Entry;	// 16 bytes
	void **pMemList_Dir_Entry=NULL;		// 8 bytes
	pthread_mutex_t lock_Dir_Entry;	// 40 bytes

	long int front_Call_Return, back_Call_Return;	// 16 bytes
	void **pMemList_Call_Return=NULL;		// 8 bytes
	pthread_mutex_t lock_Call_Return;	// 40 bytes
	...
	void Enqueue_Call_Return(void *pMemToFree_Call_Return);
};
