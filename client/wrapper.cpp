...
#define MAX_OPENED_FILE	(1024)
#define MAX_OPENED_FILE_M1	((MAX_OPENED_FILE) - 1)
#define MAX_OPENED_DIR	(256)
...
inline void Gather_FileName_Info(char szName[], int *nLenFileName, int *nLenParentDirName, unsigned long long *pFile_hash, unsigned long long *pParent_dir_hash)
{
	int i;
	...
	*nLenParentDirName = i;
	...
	if(pFile_hash)	*pFile_hash = XXH64(szName, *nLenFileName, 0);
	...
	if(strncmp(szFullPath, MYFS_ROOT_DIR, 5) == 0)	{
		...
		if(oflags & O_DIRECTORY)	{	// directory
			...
			if(pDir)	{
				...
				assert(idx_fd >= 0);
				Gather_FileName_Info(szFullPath, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), NULL);
		...
		if( (! two_args) && ( oflags & O_CREAT ) )	Gather_FileName_Info(szFullPath, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), &(pIO_Cmd->parent_dir_hash));	// create a new file
		else	{
			Gather_FileName_Info(szFullPath, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), NULL);
...
/*	
	if(strncmp(szFullPath, MYFS_ROOT_DIR, 5) == 0)	{
		...
		pIO_Cmd = (IO_CMD_MSG *)loc_buff;
		Gather_FileName_Info(szFullPathOld, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), NULL);
		idx_fs = pIO_Cmd->file_hash % pFileServerList->nFSServer;	// the index of which file server holding this file		
...
{
	int *pNumEntry, *pEntryOffsetList;
	char *pEntryNameBuff;
	...
	if(__dirp == NULL)	{
		...
		pNumEntry = (int*)( (char*)__dirp + sizeof(MYDIR) );
		pEntryOffsetList = (int*)( (char*)__dirp + sizeof(MYDIR) + sizeof(int) );
		pEntryNameBuff = (char*)__dirp + sizeof(MYDIR) + sizeof(int) + sizeof(int)*(*pNumEntry);
...
//				one_dirent.d_ino = count_readdir;
				...
				strcpy(one_dirent.d_name, pEntryNameBuff+pEntryOffsetList[__dirp->offset - 2] + 1);	// skip the d_type stored at the very beginning
				one_dirent.d_type = *((char*)(pEntryNameBuff+pEntryOffsetList[__dirp->offset - 2]));
				__dirp->offset ++;
//				one_dirent.d_ino = count_readdir;
				return &one_dirent;
		}
...
extern "C" int truncate(const char *path, off_t length)
{
	...
	if(strncmp(szFullPath, MYFS_ROOT_DIR, 5) == 0)	{
		...
		pIO_Cmd = (IO_CMD_MSG *)loc_buff;
		Gather_FileName_Info(szFullPath, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), &(pIO_Cmd->parent_dir_hash));
		idx_fs = pIO_Cmd->file_hash % pFileServerList->nFSServer;	// the index of which file server holding this file		
...
/*
	if(strncmp(p_szName, MYFS_ROOT_DIR, 5) == 0)	{
		...
		pIO_Cmd = (IO_CMD_MSG *)loc_buff;
		Gather_FileName_Info(p_szName, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), &(pIO_Cmd->parent_dir_hash));
		idx_fs = pIO_Cmd->file_hash % pFileServerList->nFSServer;	// the index of which file server holding this file		
...
int mkdir(const char *pathname, mode_t mode)
{
	...
	if(strncmp(szFullPath, MYFS_ROOT_DIR, 5) == 0)	{
		...
		Gather_FileName_Info(szFullPath, &(pIO_Cmd->nLen_FileName), &(pIO_Cmd->nLen_Parent_Dir_Name), &(pIO_Cmd->file_hash), &(pIO_Cmd->parent_dir_hash));	// create a new file
		idx_fs = pIO_Cmd->file_hash % pFileServerList->nFSServer;	// the index of which file server holding this file
...
size_t test_read_client(int fd, char *buf, size_t size)
{
	...
	if(size > DATA_COPY_THRESHOLD_SIZE)	{
		mr_loc_buf = pClient_qp[0]->IB_RegisterBuf_RW_Local_Remote(buf, size);
		pIO_Cmd->rem_buff = mr_loc_buf->addr;
		pIO_Cmd->rkey = mr_loc_buf->rkey;
	}
	else	{
