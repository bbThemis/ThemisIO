...
int main(int argc, char *argv[])
{
	...
	d_time_OP = tm0.tv_sec + 0.000001;
	if(world_rank == 0)	printf("DBG> T0 = %20.3lf\n", d_time_OP);
	...
	while(d_time_Accu < T_Max)   {
		...
		if(nIter % 1 == 0)    {
			...
			if(d_time_OP >= T_WINDOW)       {
				...
				if(world_rank == 0)     printf("T = %5.1lf    Bandwidth = %6.3lf GB/s    %6.3lf M IOPS     T_Reduce = %5.0lf us     nOP_Accu = %6.2lf\n",
					d_time_Accu, (nIter_Sum-nIter_Sum_Save)*0.000001*nByste_RW*0.001/d_time_OP, (nIter_Sum-nIter_Sum_Save)*0.000001/d_time_OP, d_time, nIter_Sum*0.000001);
				tm1.tv_sec = tm2.tv_sec;
				tm1.tv_usec = tm2.tv_usec;
