...
std::unordered_map<int, file_flags> fd_file_name_map;
...
extern "C" void* ishank_mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset){
    ...
    void * ans = mmap(NULL, length, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    
    if(fd < 0) return ans;

    file_flags info = fd_file_name_map[fd];

    int newfd = open(info.filename, info.flags, info.mode);
    assert(newfd > 0);
    ...
    while(remaining > 0){
        ssize_t num_read = pread(newfd, ans, remaining, offset);
    ...
    populate_map(length, original, newfd);
...
int main(int argc, char **argv) {
    ...
    info.mode = 0644;
    fd_file_name_map.emplace(fd, info);
